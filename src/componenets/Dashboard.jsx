        ...
        <div className="bg-white border rounded-xl shadow p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-3">
              <Link to="/my-account" className="px-4 py-2 rounded-lg border text-blue-700 border-blue-200">Edit Profile</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <section className="bg-white border rounded-xl shadow p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/cart" className="text-sm text-blue-700 hover:underline">Go to Cart</Link>
            </div>
            {orders.length === 0 ? (
              <p className="text-gray-500 text-sm">No orders yet. Place your first order from the <Link to="/books" className="text-blue-700 underline">Books</Link> page.</p>) : (
                ...
                {orders.slice(0,5).map((o) => (
                  <li key={o.id} className="py-3 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-700 mt-1">Items: {o.items?.length || 0}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">₹{o.total?.toFixed(2)}</p>
                      <span className="inline-block mt-1 text-xs px-2 py-1 rounded-full border {o.status === 'Delivered' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-yellow-50 border-yellow-200 text-yellow-700'}">
                        {o.status || 'Processing'}
                      </span>
                    </div>
                  </li>
          ...
          <section className="bg-white border rounded-xl shadow p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/books" className="text-sm text-blue-700 hover:underline">Browse Books</Link>
            </div>
            {orders.length === 0 ? (
